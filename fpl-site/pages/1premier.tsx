  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-blue-200 via-white to-purple-100 text-[#37003c]">
      ...
      <section className="p-6">
        <h2 className="text-3xl font-bold mb-4">Premier League Table</h2>
              ...
              <tr>
                {data[0] && Object.keys(data[0]).map((key) => (
                  key !== "Owner" && key !== "Title Reward" && (
                    <th key={key} className="px-3 py-2 text-left font-semibold text-xs bg-gray-100 text-[#37003c]">
                      {key}
                    </th>
                  )
                ))}
                    ...
                    if (key === "Position") {
                      return (
                        <td key={j} className={`px-3 py-2 text-sm border-b border-gray-400 ${getCellStyle(key, val, row)}`}>
                          ...
                          <div className="text-xs italic text-purple-700">{positionLabels[val]}</div>
                        </td>
                      );
                    }
                    ...
                    if (key === "Team") {
                      return (
                        <td key={j} className={`px-3 py-2 text-sm border-b border-gray-400 ${getCellStyle(key, val, row)}`}>
                          ...
                          <div className="text-xs text-gray-600">{row.Owner}</div>
                        </td>
                      );
                    }
