...
export default function Premier() {
  ...
  const getCellStyle = (key, val, row) => {
    ...
    if (
      ["Chips Used", "Free Hit", "Wildcard 1", "Wildcard 2", "Triple Captain", "Bench Boost", "AssMan"].includes(key)
    ) {
      if (val.includes("GW")) return "bg-red-200";
      if (val.includes("Expired")) return "bg-orange-200";
      if (val.includes("Available")) return "bg-green-200";
    ...
    if (key === "GW Transfer Hit") {
      const values = data.map(r => parseFloat(r[key])).filter(n => !isNaN(n) && n > 0);
    ...
    if (key === "GW Transfers Made" || key === "Total Transfers Made") {
      const values = data.map(r => parseFloat(r[key])).filter(n => !isNaN(n) && n > 0);
      if (values.length === 0) return "";
      const sorted = [...values].sort((a, b) => b - a);
      if (num > 0 && (num === sorted[0] || num === sorted[1] || num === sorted[2])) {
    ...
    <main className="min-h-screen bg-gradient-to-b from-blue-200 via-white to-purple-100 text-[#37003c]">
      <div className="bg-gradient-to-r from-blue-300 via-blue-400 to-purple-700 text-white p-6 shadow-lg">
        ...
        <nav className="mt-4 flex gap-4">
          ...
          <Link href="/championship"><button className="bg-[#32FF6A] text-[#37003c] px-4 py-2 rounded font-semibold shadow">Premier</button></Link>
                    ...
                    if (key === "Team") {
                      return (
                        <td key={j} className={`px-3 py-2 text-sm border-b border-gray-400 ${getCellStyle(key, val, row)}`}>
                          ...
                          <div className="text-center text-xs text-gray-600">{row.Owner}</div>
                        </td>
                      );
                    }
